...
// distances of a graph's edges ' @param numNodes, the number of nodes in the
// graph '
//@param defaultEdgeWeight, the default edge weight if an edge is not-specified
//(default value 999999) ' @return a List with 1) the list of tour vertices, and
// 2) the total tour cost > NULL
...
Rcpp::List ChristofidesRunner(std::vector<int> arcSources,
  ...
  // indicates the target and source and disjoint ATTENTION: TSP implemented
  // using fullgraph, so remember that all edges exist
...
Rcpp::List GreedyTSPRunner(std::vector<int> arcSources,
  ...
  // start node to each of the nodes, with "-1" used as a placeholder to
  // indicates the target and source and disjoint ATTENTION: TSP implemented
  // using fullgraph, so remember that we can't test this for large n.
  FullGraph g(numNodes);
