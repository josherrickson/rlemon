...
//' @description Due to how LEMON operates, and to make the interfaces similar
// to the actual program, ' graphs in rLEMON use the following 2 vectors: '
// arcSources, and arcTargets ' to acquire information about how the graph is
// structured ' each edge in a graph will consist of a (node id, node id) pair,
// which is represented ' by the ith entry in both arcSources and arcTargets. '
// The nodes are 0-indexed, and must be sequential. ' numNodes must be equal to
// the number of nodes in the graph, as otherwise there ' are potentials to
// error out. ' As an example, the cycle graph on three vertices would have the
// following input: ' arcSources <- c(0,1,2) ' arcTargets <- c(1,2,0) ' numNodes
//<- 3 ' Here, the edges are (0,1), (1,2), (2,0) > NULL
...
// edges ' @param arcTargets, a vector corresponding to the destination nodes of
// a graph's edges ' @param numNodes, the number of nodes in the graph ' @return
// a boolean, stating if a graph has some property > NULL
...
// edges ' @param arcTargets, a vector corresponding to the destination nodes of
// a graph's edges ' @param numNodes, the number of nodes in the graph ' @return
// an integer, counting some property of a graph > NULL
...
// edges ' @param arcTargets, a vector corresponding to the destination nodes of
// a graph's edges ' @param numNodes, the number of nodes in the graph ' @return
// a List, returning some set of nodes or edges in a graph > NULL

//' @rdname Connectivity-Algorithms-3
//' @return `getBipartitePartitions` returns an R List containing (1) A boolean
// stating if the graph is bipartite, and (2) an std::vector of length numNodes,
// containing the partition for each node ' @export
// [[Rcpp::export]]
...
//' @rdname Connectivity-Algorithms-3
//' @return `getAndCheckTopologicalSort` returns an R List containing (1) A
// boolean stating if the graph is a dag, and (2) a vector of length numNodes,
// containing the index of vertex i in the ordering at location i ' @export
// [[Rcpp::export]]
...
//' @rdname Connectivity-Algorithms-3
//' @return `getTopologicalSort` returns a vector of length numNodes, containing
// the index of vertex i in the ordering at location i. ' @export
// [[Rcpp::export]]
...
//' @rdname Connectivity-Algorithms-1
//' @description `IsStronglyConnected` returns if a graph is strongly connected
// or not ' @export
// [[Rcpp::export]]
...
//' @rdname Connectivity-Algorithms-1
//' @description `IsBiNodeConnected` returns if a graph is bi-node connected or
// not ' @export
// [[Rcpp::export]]
...
//' @rdname Connectivity-Algorithms-1
//' @description `IsBiEdgeConnected` returns if a graph is bi-edge connected or
// not ' @export
// [[Rcpp::export]]
...
int IsSimpleGraph(std::vector<int> arcSources, std::vector<int> arcTargets,
  ...
  //           us the number of nodes in the directed graph and the starting
  //           node for Bellman Ford
  // Returns: A boolean stating if the graph is simple, i.e. it has no loop
  // arcs/edges or parallel arcs/edges
  ListGraph g;
  std::vector<ListGraph::Node> nodes;
  for (int i = 0; i < numNodes; ++i) {
    ListGraph::Node n = g.addNode();
...
int IsEulerian(std::vector<int> arcSources, std::vector<int> arcTargets,
  ...
  //           us the number of nodes in the directed graph and the starting
  //           node for Bellman Ford
  // Returns: A boolean stating if the graph is simple, i.e. it has an eulerian
  // cycle
  ListGraph g;
  std::vector<ListGraph::Node> nodes;
  for (int i = 0; i < numNodes; ++i) {
...
//' @description `CountBiEdgeConnectedComponents` returns how many Bi-Edge
// Connected Components a graph has. ' @export
// [[Rcpp::export]]
...
//' @rdname Connectivity-Algorithms-2
//' @description `CountConnectedComponents` returns how many Connected
// Components a graph has. ' @export
// [[Rcpp::export]]
int CountConnectedComponents(std::vector<int> arcSources,
  ...
  //           us the number of nodes in the directed graph and the starting
  //           node for Bellman Ford
  // Returns: An integer stating the number of connected components of this
  // graph
  ListGraph g;
  std::vector<ListGraph::Node> nodes;
  for (int i = 0; i < numNodes; ++i) {
...
//' @rdname Connectivity-Algorithms-2
//' @description `CountBiNodeConnectedComponents` returns how many Bi-Node
// Connected Components a graph has. ' @export
// [[Rcpp::export]]
int CountBiNodeConnectedComponents(std::vector<int> arcSources,
  ...
  //           us the number of nodes in the directed graph and the starting
  //           node for Bellman Ford
  // Returns: An integer stating the number of bi-node connected components of
  // this graph
  ListGraph g;
  std::vector<ListGraph::Node> nodes;
  for (int i = 0; i < numNodes; ++i) {
...
//' @rdname Connectivity-Algorithms-2
//' @description `CountStronglyConnectedComponents` returns how many Strongly
// Connected Components a graph has. ' @export
// [[Rcpp::export]]
...
//' @rdname Connectivity-Algorithms-3
//' @description `FindStronglyConnectedComponents` returns a vector containing
// the component number of each node ' @export
// [[Rcpp::export]]
...
//' @rdname Connectivity-Algorithms-3
//' @description `FindStronglyConnectedCutArcs` returns an R List containing 1)
// A list of cut arc sources, and 2) A list of cut arc destinations ' @export
// [[Rcpp::export]]
Rcpp::List FindStronglyConnectedCutArcs(std::vector<int> arcSources,
                                        std::vector<int> arcTargets,
                                        int numNodes) {
  // Returns: Two std::vectors containing the source and destination of the cut
  // arcs
  ListDigraph g;
  std::vector<ListDigraph::Node> nodes;
  for (int i = 0; i < numNodes; ++i) {
...
//' @rdname Connectivity-Algorithms-3
//' @description `FindBiEdgeConnectedCutEdges` returns an R List containing 1) A
// list of cut edge sources, and 2) A list of cut edge destinations ' @export
// [[Rcpp::export]]
...
//' @rdname Connectivity-Algorithms-3
//' @description `FindBiNodeConnectedCutComponents` returns a vector containing
// the component number of each cut arc. ' @export
// [[Rcpp::export]]
...
//' @rdname Connectivity-Algorithms-3
//' @description `FindBiNodeConnectedCutNodes` returns a vector containing the
// cut nodes ' @export
// [[Rcpp::export]]
...
//' @rdname Connectivity-Algorithms-3
//' @description `FindConnectedComponents` returns a vector containing the
// component number of each node ' @export
// [[Rcpp::export]]
...
//' @rdname Connectivity-Algorithms-3
//' @description `FindBiEdgeConnectedComponents` returns a vector containing the
