...
using NS = NetworkSimplex<ListDigraph, Capacity, Cost>;

vector<Flow> networkSimplex(vector<int> node_supplies, vector<int> arc_src, vector<int> arc_targ, vector<int> arc_costs, vector<int> arc_capacities)
{
    ...
    NS ns(g);
    ns.costMap(costs).upperMap(capacities).lowerMap(lower_capacities).supplyMap(supplies);
    ...
    case NS::OPTIMAL:
        ...
        Rcout << "Total Cost: " << ns.totalCost() << endl;
        break;
    case NS::UNBOUNDED:
